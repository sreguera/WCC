...
Tacky generator for Chapter 9 of "Writing a C Compiler".
...
is_tacky_program(program(FunDecls)) :-
    forall(member(Decl, FunDecls), is_tacky_function(Decl)).

is_tacky_function(function(Name, Params, Body)) :-
    atom(Name),
...
is_tacky_instruction(label(Identifier)) :-
    atom(Identifier).
is_tacky_instruction(funcall(Identifier, Args, Dst)) :-
    atom(Identifier),
    forall(member(Arg, Args), is_tacky_value(Arg)),
    is_tacky_value(Dst).
...
tack_program(program(FunDecls), program(FunDefsTacky)) :-
    reset_gensym,
    convlist(tack_function, FunDecls, FunDefsTacky).

tack_function(fun_decl(Name, Params, Body), function(Name, Params, Instructions)) :-
...
decl_insts(var_decl(Var, Exp), I0, Is) :-
    (   Exp = none
    ->  I0 = Is
    ;   exp_insts(Exp, Result, I0, I1),
        I1 = [copy(Result, var(Var))|Is]
    ).
% Do nothing. Functions at the top level are treated above in tack_function.
% This would only be called in block declarations, and those have already been
% checked in semantics to have no body.
decl_insts(fun_decl(_Id, _Params, none), Is, Is).
...
exp_insts(funcall(Id, Args), Dest, I0, Is) :-
    args_insts(Args, Results, I0, I1),
    mk_tmpvar(Dest),
    I1 = [funcall(Id, Results, Dest)|Is].
...
args_insts([Arg|Args], [Result|Results], I0, Is) :-
    exp_insts(Arg, Result, I0, I1),
    args_insts(Args, Results, I1, Is).
args_insts([], [], Is, Is).
...
test(p5) :-
    ProgramIn = program([fun_decl(main, [], block([
        d(var_decl('var.a.1', unary(negate, constant(5)))),
...
test(p9) :-
    ...
    tack(ProgramIn, ProgramOut),
    assertion(is_tacky(ProgramOut)),
    assertion(ProgramOut = program([
        function(main, [], [
            funcall(foo, [constant(2), constant(1)], var('tmp.1')),
            return(var('tmp.1')),
            return(constant(0))
        ]),
