...
Parser for Chapter 9 of "Writing a C Compiler".
...
is_program_ast(program(FunctionDeclarations)) :-
    forall(member(Decl, FunctionDeclarations), is_fun_decl_ast(Decl)).
...
program(program(FunctionDeclarations)) -->
    sequence(function_declaration, FunctionDeclarations).
...
variable_declaration(var_decl(Name, Init)) -->
    [int, identifier(Name)],
...
function_declaration(fun_decl(Name, Params, Block)) -->
    [int, identifier(Name)],
    param_list(Params),
    ( [';'], {Block = none}
    ; block(Block)
...
test(parse) :-
    ...
    assertion(Program = program([fun_decl(main, [], block([s(return(constant(2)))]))])).

test(parse2) :-
    lex("int main(void) { return ~(-2); }", Tokens),
    parse(Tokens, Program),
    assertion(is_ast(Program)),
    assertion(Program = program([fun_decl(main, [], block([s(return(unary(complement, unary(negate, constant(2)))))]))])).

test(parse3) :-
    lex("int main(void) { return 1 + 2; }", Tokens),
    parse(Tokens, Program),
    assertion(is_ast(Program)),
    assertion(Program = program([fun_decl(main, [], block([s(return(binary(add,constant(1),constant(2))))]))])).

test(parse5) :-
    lex("int main(void) { int a = 5; int b; b = a - 3; return b; }", Tokens),
    parse(Tokens, Program),
    assertion(is_ast(Program)),
    assertion(Program = program([fun_decl(main, [], block([
...
test(decl) :-
    ...
    Decl = var_decl(b, binary(add, constant(3), unary(post_incr, var(a)))).
